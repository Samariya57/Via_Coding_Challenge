	...
	#day = "Tuesday"
		for i in range(1440):
			if i<30:
				...
				cursor.execute(sum_stmt_10.format(week_day=day,minute_left=i+1,minute_right=i+31))
				right_sum = int(cursor.fetchone()[0])
			print(i+1,"%.7f"%1 if (right_sum/left_sum)>1 else ("%.7f" % (right_sum/left_sum)) )
			cursor.execute(insert_stmt.format(week_day=day),(i+1,"%.7f"%1 if (right_sum/left_sum)>1 else ("%.7f" % (right_sum/left_sum))))
			db.commit()	
